...
layui.use(['form', 'jquery'], function () {
    ...
    form.on('submit(nbv5reg)', function (data) {
        if (isOpenRegister) {
            data.field.npUsername = data.field.nbv5regUsername;
            data.field.npPassword = md5(data.field.nbv5regPassword);
            data.field.npNickname = data.field.nbv5regNickname;
            data.field.npMail = data.field.nbv5regMail;
            data.field.code = data.field.mailCode;
            $.post("/registration", data.field, function (resp) {
                    if (resp.code === 200) {
                        layer.msg("注册成功！");
                        setTimeout(function () {
                            location.href = "/np-login" || resp.data;
                        }, 1000);
                    } else {

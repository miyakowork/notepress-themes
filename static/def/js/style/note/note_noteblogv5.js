...
layui.define(['laytpl', 'timeago', 'laypage'], function (exports) {
    ...
    var _tpl = '' +
        ...
        '       </div>' +
        '       <div class="layui-col-lg-offset4 layui-col-md-offset4 layui-col-lg4 layui-show-md-inline-block  layui-hide-sm layui-hide-xs" style="text-align: right;">' +
        ...
        '                <h3 class="layui-timeline-title"><i class="fa fa-book"></i>&nbsp;封面</h3>' +
        '            </div>' +
        '        </li>' +
        ...
        '{{item.title}}' +
        '<small class="timeago" style="margin-left: 10px;font-style: italic;color: #cccccc;" datetime="{{npfront.timeAgo(item.gmtCreate) }}">' +
        '</small>' +
        '</h3>' +
        ...
        '<i class="layui-icon layui-timeline-axis">&#xe63f;</i>' +
        '<div class="layui-timeline-content layui-text">' +
        '<h3 class="layui-timeline-title"><i class="fa fa-book"></i>&nbsp;页末</h3>' +
        '</div>' +
    ...
    exports("notenbv5", function (pageObj, words) {
        po = pageObj;
        var obj = {
            pageObj: pageObj,
...
function searchNote(e) {
    ...
    if (keyNum === 13) {
        ...
        location.href = "/note?s=" + btoa(encodeURIComponent(words));
    }
}
...
function initPage(pageObj, page) {
    page.render({
        ...
        , jump: function (obj, first) {
            if (!first) {
                var words = npfront.getQueryString(s);
                words = words === null ? "" : words;
                location.href = "/note?s=" + words + "&p=" + obj.curr
            }
        }
    });
}

...
layui.define(['laytpl', 'laypage', 'carousel', 'timeago', 'affixside', 'toparea', 'col39list'], function (exports) {
    ...
    var tpl = layui.laytpl,
        ...
        col39list = layui.col39list,
        affixside = layui.affixside;


    var col_l3r9 =
        '<div id="article-list-col39" class="layui-row layui-col-space20 col39" style="margin-bottom: 20px;">' +
        ...
        '<label style="float: right;cursor:pointer;color: #f44336;" id="more39">更多<i class="fa fa-angle-double-right"></i></label></blockquote> ' +
        '<div id="content-html">' +
        contentHtml +
        '</div>' +
    ...
    exports('col39', function (obj) {
        var $mainBody = $("#main-body");
        tpl(col_l3r9).render(obj, function (html) {
            $mainBody.prepend(html);
            initCol39Page(laypage, tpl, timeago, obj.nonePage);
            timeago.render($('.timeago'));
            stickySideBar(obj.nonePage);
            initCarousel(carousel);

            $mainBody.on("click", "#more39", function () {
                $.ajax({
                    url: "/index/next",
                    data: {
                        pn: 1,
...
function initCol39Page(laypage, laytpl, timeago, page) {
...
function more39(col39list, objParam) {
    $("#article-list-col39>.layui-col-md9").remove();
    col39list(objParam);
}
